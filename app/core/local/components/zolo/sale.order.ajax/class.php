...
class SaleOrderAjax extends \CBitrixComponent implements Controllerable
{
    ...
    public function createOrderAction(array $data): array
    {
        ...
        $orderId = (new OrderHelper)->createOrder($userId, $data);

		if ($this->addBonusPayments((int)$data['bonuses_subtract'], (int)$orderId, (int)$userId) {
			// место для логгера.
		}
	 ...
	 * 
	 */
	private function addBonusPayments(int $bonusPoints, int $orderId, int $userId): bool
	{
        global $USER;

		return (new OrderHelper)->addBonusPayments($bonusPoints, $orderId, $userId);
	}
	...
	protected function obtainBasket()
	{
		...
		foreach ($this->calculateBasket as $basketItem)
		{
            ...
            {
                $bonuses = PriceTable::getList([
                    'filter' => [
                        '=PRODUCT_ID' => $arBasketItem["PRODUCT_ID"],
                        '=CATALOG_GROUP_ID' => $this->user->catalogGroupId,
                    ],
                    'limit' => 1,
                ])->fetch();
                $arBasketItem["BONUSES"] = (float) $bonuses["PRICE"];
            }
	...
	protected function obtainTotal()
	{
        ...
        $arResult['ORDER_DELIVERY_DATE'] = (new Main\Type\DateTime)->add(OrderService::DELIVERY_OFFSET . ' day')->format('d.m.Y');
	}
