...
class UserGroupMigrationCreator extends MigrationCreator
{
    public function getBuilder($method)
    {
        $class = '\\QSoft\\Migration\\Builder\\'.ucfirst($method).'UserGroupBuilder';
        return new $class();
    }

    protected function generate(BuilderInterface $builder, $data)
    {
        if (empty($data['consolidatedUserGroupId']))
            $data['consolidatedUserGroupId'] = $this->identityRepository->generateId();
